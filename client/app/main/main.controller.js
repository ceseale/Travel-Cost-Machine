angular.module('201510MvpApp').controller('MainCtrl', function ($scope, $http, $timeout, $mdToast, $mdDialog) {
  $scope.loading = false;
  $scope.userCar = {};
  $scope.makes = [];
  $scope.models = [];
  ...
  $scope.showSimpleToast = function (text) {
      $mdToast.show($mdToast.simple().content(text || 'Changing Network').position($scope.getToastPosition()).hideDelay(3000));
    }
    // Geo JS Stuff
  ...
  function makeContour(data, layer) {
    ...
    var contour = layer.createFeature('contour').data(data.position || data.values).style({
      opacity: 0.5
    }).contour({
  ...
  var vglLayer = map.createLayer('feature', {
    renderer: 'vgl'
  });
  ...
  // Adding d3 layer
  var dot = layer.createFeature('point').data($scope.currentPoint).style('radius', 5).style('fillColor', function () {
    return 'steelblue';
  }).position(function (d) {
    return d;
  });
  ...
  function pulse() {
    (function repeat() {
      circle = circle.transition().duration(2000).attr("stroke-width", 0).attr("r", 5).transition().duration(100).attr('stroke-width', 0).attr("r", 20).ease('linear').each("end", repeat);
    })();
  }
